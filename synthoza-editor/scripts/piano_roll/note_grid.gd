...
func _draw() -> void:
	var snap_per_note := maxi(_piano_roll.snap_value / _piano_roll.note_value, 1)
	var note_size := _piano_roll.note_size
	var snap_size := note_size / Vector2(maxi(_piano_roll.snap_value / 4, 1), 1)
	...
	while cursor.x < size.x:
		var snap_signature := posmod(grid_num.x, _piano_roll.notes * snap_per_note)
		draw_line(
